...
class RestrictionEnzyme:
    ...
    def lc(self, oh1: str, oh2: str) -> Decimal:
        """Ligation counts between oh1 and oh2."""
        return self.ligation_counts[(oh1, oh2)]

    def _init_stack_element(self, oh: str) -> _StackElement:
        """Init method for the first set of overhangs in max_gg_prob."""

        coh = _reverse_complement(oh)  # Complementary overhang.

        # Get three types of ligation counts.
        oh_oh = self.lc(oh, oh)
        oh_coh = self.lc(oh, coh)
        coh_coh = self.lc(coh, coh)
    ...
    def gg_prob(self, overhangs: list[str], has_complements: bool = False
                ) -> Decimal:
        """Golden Gate Assembly efficiency."""
        if not has_complements:
            complements = [_reverse_complement(oh) for oh in overhangs]
            # overhangs += list(reversed(complements))
            # overhangs += complements
            new_overhangs = []
            for oh, coh in zip(overhangs, complements):
                new_overhangs.extend([oh, coh])
            overhangs = new_overhangs

        prob = Decimal(1.0)

        for oh1 in overhangs:
            tot = sum(self.ligation_counts[(oh1, oh2)] for oh2 in overhangs)
            oh1_complement = _reverse_complement(oh1)
            p_oh1 = self.ligation_counts[(oh1, oh1_complement)] / tot
            prob *= p_oh1

        return prob

    def max_gg_prob(self, breakpoints: list[BreakPoint],
                    thresh: Decimal = Decimal(1.0)) -> tuple[Decimal,
                                                             list[str]]:
        ...
        while comp_stack:
            ...
            for next_oh in overhang_sets[oh_index]:
                next_coh = _reverse_complement(next_oh)

                # Next denominators to track. Not including denoms for next_oh
                # and next_coh.
                next_denoms = [d + self.lc(oh, next_oh)
                               + self.lc(oh, next_coh)
                ...
                n_denom = sum(self.lc(oh, next_oh) for oh in next_overhangs)
                n_denom = Decimal(n_denom)  # avoids mypy error
                nc_denom = sum(self.lc(oh, next_coh) for oh in next_overhangs)
                nc_denom = Decimal(nc_denom)  # avoids mypy error
                next_denoms += [n_denom, nc_denom]

                noh_ncoh = self.lc(next_oh, next_coh)
                ...
                if next_prob > max_gg_prob:
                    if oh_index+1 == len(breakpoints):
                        # We're at the end of breakpoints, so this is a valid
                        # set.
